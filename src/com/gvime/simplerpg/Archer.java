...
public class Archer extends Hero {
    ...
    private final double CRIT_PER_LEVEL = 0.004; // 0.4% crit chance per level

    /*
     ARCHERS STARTS WITH 10% CRIT CHANCE
     AND RECEIVE 0,4% PER LEVEL (TOTAL OF 50% AT LV 100)
     (THE RANGED DPS OF THE TEAM)
     */

    //CREATES LV 1 ARCHER
    public Archer(String name) {
        super(name, 1, 100, 15, 5, 0.7, 0.08, 0.1, 2.0);
    }

    //CREATES AN ARCHER WITH A GIVEN LEVEL
    public Archer(String name, int lv) {
        super(name, lv, 100, 15, 5, 0.7, 0.05, 0.1, 2.0);
     ...
     */
    public Archer(String name, int lv, double hp, double att, double def, double spe, double eva, double critChance, double critDamage) {
